...
* 삽입: O(N)$O(N)$ (리스트의 위치를 찾아야 하므로)
* 삭제: O(1)$O(1)$ (리스트의 시작 부분에서 제거하므로)
* 최상위 조회: O(1)$O(1)$ 
...
* Java의 `PriorityQueue`는 유연한 비교 기능을 제공하여 최소 힙과 최대 힙을 구현할 수 있습니다. 객체의 우선순위에 따라 정렬하여 다양한 문제를 해결하는 데 유용합니다.
* Python의 `heapq`는 간단한 인터페이스를 제공하며, 최소 힙을 기본으로 지원하지만, 음수 값을 사용하여 최대 힙을 구현할 수도 있습니다.
